...
@Composable
fun CommentsScreen(navController: NavController, vm: PicViewModel, postId: String) {
    var commentText by rememberSaveable { mutableStateOf("") }
    val focusManager = LocalFocusManager.current

    val comments = vm.comments.value
    val commentsProgress = vm.commentsProgress.value
    val currentUsername = vm.userData.value?.username
                ...
                items(items = comments) { comment ->
                    CommentRow(
                        ...
                        onDeleteComment = { commentId ->
                            vm.deleteComment(commentId = commentId, postId = postId)
                        },
